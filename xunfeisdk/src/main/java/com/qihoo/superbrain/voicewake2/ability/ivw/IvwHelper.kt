package com.qihoo.superbrain.voicewake2.ability.ivw
...
import com.qihoo.superbrain.voicewake2.media.audio.AudioRecorder
import com.qihoo.superbrain.voicewake2.media.audio.RecorderCallback
import com.qihoo.superbrain.voicewake2.tool.mainThread
import java.io.*
...
class IvwHelper(
    ...
    companion object {
        ...
        @JvmStatic
        fun createKeywordFile(context: Context, keyword: String): String {
            val file = File(context.externalCacheDir, "keyword.txt")
            if (file.exists()) {
                file.delete()
            }
            val binFile = File("${context.externalCacheDir}/process", "key_word.bin")
            if (binFile.exists()) {
                binFile.delete()
            }
//            if (file.exists() && binFile.exists()) {
...
//            val bufferedWriter = BufferedWriter(FileWriter(file))
    ...
    val recorder: AudioRecorder by lazy {
    ...
    fun startAudioRecord(context: Context, keyword: String, nCmThreshold: Int = 900) {
        val filePath = createKeywordFile(context, keyword)
        val keywordSize = keyword.trim().split(";").count()
        startAudioRecord(filePath, keywordSize, nCmThreshold)
    }
    ...
    private fun initParams(
        ...
        AiHelper.getInst().setLogLevel(LogLvl.OFF)
...
//            Log.w(TAG, "open ivw specifyDataSet error code ===> $ret")
        ...
        val params = AiRequest.builder().apply {
            param(
                "wdec_param_nCmThreshold",
                nCmThresholdBuilder.toString()
            ) //门限值	, 最小长度:0, 最大长度:1024
            param("gramLoad", true)
        }.build()
...
//        )
        for (item in esrData) {
            ...
            if (tempKey.contains("func_wake_up") || tempKey.contains("func_pre_wakeup")) {
                mainThread {
                    callback.onAbilityResult(tempKey, tempValue)
                }
            }
        }
